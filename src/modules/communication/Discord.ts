  ...
  private async sendMessage (channelId: string, message: string): Promise<void> {
    ...
    if (channel !== null && channel.isTextBased()) {
      try {
        ...
        console.error(`Erreur lors de l'envoi d'un message à ${channelId}:`, error)
      }
    } else {
      console.error(`Le canal ${channelId} n'est pas valide ou n'est pas textuel.`)
    }
  }
      ...
      this.client.on('messageCreate', async (message) => {
        ...
        if (content === '/leave') {
          ...
          if (index !== -1) {
            ...
            message.reply('Ce canal n\'est pas inscrit pour les notifications.').catch(console.error)
          }
        }
...
- /help : Afficher l'aide.`).catch(console.error)
        }
      })
    })
  }
